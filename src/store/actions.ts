...
export type GameAction =
  ...
  | {                                                        // one drone→task manipulation while building/editing a tactical plan (logging only, no state change)
      ...
      missionId: string
      op: 'assign' | 'chain' | 'remove' | 'unassign'
      droneId: string
      taskId: string | null
  ...
  | { type: 'CONFIRM_FAILURE_RECOVERY'; missionId: string; taskAssignments: Record<string, string[]>; droneSequences?: Record<string, string[]>; wasAgentSuggested?: boolean }
