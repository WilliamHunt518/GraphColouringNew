...
We develop a simulated search-and-rescue scenario using a heterogeneous drone swarm, and characterise how operators interact with the two-tier assistive system compared to a fully manual baseline. In a within-subjects counterbalanced design (agent-assisted vs.\ manual), we address the following research questions:
...
\begin{itemize}
  ...
  \item \textbf{RQ3}: What advantage does agent-assisted allocation confer relative to manual allocation, in terms of score, Green utilisation, and mission completion time?
\end{itemize}
...
The platform is a browser-based simulation designed for a two-monitor setup. The \emph{primary window} (Figure~\ref{fig:primary}) runs on the main monitor and provides the command console; the \emph{map window} runs on a second monitor and provides the operational map and tactical planner.
...
\begin{tabular}{lccc}
\toprule
Condition & Score $S$ & Green efficiency & Mean mission time (s) \\
\midrule
Agent  & -- & -- & -- \\
Manual & -- & -- & -- \\
\bottomrule
...
\begin{tabular}{lccc}
\toprule
Condition & TLX$_\mu$ & Trust (Strategic Agent) & Trust (Tactical Agent) \\
\midrule
Agent  & -- & -- & -- \\
Manual & -- & -- & -- \\
\bottomrule
...
[To be written. Key themes: whether agent-assisted allocation produces a measurable performance advantage and under what situational conditions that advantage is largest; characterisation of when operators elect to use the agent versus allocate manually, and whether this is sensitive to reserve depth and mission complexity; implications for the design of selective autonomy interfaces in human-swarm coordination.]

% -----------------------------------------------------------------
\section{Conclusion}\label{sec:conclusion}

We have presented a user study examining how operators interact with a hierarchically structured autonomous assistance system in a simulated search-and-rescue context. In a within-subjects counterbalanced design, we compare agent-assisted and manual allocation to assess the performance benefit of the assistive system, characterise when operators choose to engage it, and estimate the advantage conferred by agent-assisted decisions relative to manual ones. The core theoretical contribution is an empirical account of selective autonomy use across a two-tier hierarchy: prior work has examined depth and breadth of autonomy within a single decision layer; the present work asks how operators distribute and selectively engage autonomous assistance structured across two qualitatively different decision levels. The practical contribution is a characterisation of the situational factors that predict agent engagement---a question of direct relevance to the design of next-generation command-and-control interfaces for autonomous platforms.
